...
SELECT
    $1
FROM
    @UNI_KLAUS_ZMD/sweatsuit_sizes.txt;
SELECT
    $1
FROM
    @uni_klaus_zmd/product_coordination_suggestions.txt;
CREATE FILE FORMAT zmd_file_format_1 record_delimiter = '^';
SELECT
    $1
FROM
    @uni_klaus_zmd/product_coordination_suggestions.txt (file_format => zmd_file_format_1);
CREATE FILE FORMAT zmd_file_format_2 field_delimiter = '^';
SELECT
    $10
FROM
    @uni_klaus_zmd/product_coordination_suggestions.txt (file_format => zmd_file_format_2);
CREATE
    OR REPLACE FILE FORMAT zmd_file_format_3 record_delimiter = '^' field_delimiter = '=';
CREATE VIEW zenas_athleisure_db.products.SWEATBAND_COORDINATION AS
SELECT
    $1 product_code,
    $2 has_matching_sweatsuit
FROM
    @uni_klaus_zmd/product_coordination_suggestions.txt (file_format => zmd_file_format_3);
SELECT
    *
FROM
...
FROM
    @uni_klaus_zmd/sweatsuit_sizes.txt (file_format => zmd_file_format_1)
WHERE
    sizes_available != '';
SELECT
    *
...
CREATE
    OR REPLACE FILE FORMAT zmd_file_format_2 record_delimiter = ';' field_delimiter = '|' trim_space = TRUE;
CREATE VIEW zenas_athleisure_db.products.SWEATBAND_PRODUCT_LINE AS
SELECT
    REPLACE($1, chr(13) || chr(10)) product_code,
    $2 headband_description,
    $3 wristband_description
FROM
    @UNI_KLAUS_ZMD/swt_product_line.txt (file_format => zmd_file_format_2);
SELECT
    *
FROM
...
SELECT
    REPLACE(
        REPLACE(REPLACE(UPPER(RELATIVE_PATH), '/'), '_', ' '),
        '.PNG'
    ) AS product_name
FROM
    directory(@uni_klaus_clothing);
...
INSERT INTO
    ZENAS_ATHLEISURE_DB.PRODUCTS.SWEATSUITS (COLOR_OR_STYLE, DIRECT_URL, PRICE)
VALUES
    (
        '90s',
        ...
        'Burgundy',
        'https://uni-klaus.s3.us-west-2.amazonaws.com/clothing/burgundy_sweatsuit.png',
        65
    ),(
        'Charcoal Grey',
        'https://uni-klaus.s3.us-west-2.amazonaws.com/clothing/charcoal_grey_sweatsuit.png',
        65
    ),(
        'Forest Green',
        'https://uni-klaus.s3.us-west-2.amazonaws.com/clothing/forest_green_sweatsuit.png',
        65
    ),(
        'Navy Blue',
        'https://uni-klaus.s3.us-west-2.amazonaws.com/clothing/navy_blue_sweatsuit.png',
        65
    ),(
        'Orange',
        ...
        'Pink',
        'https://uni-klaus.s3.us-west-2.amazonaws.com/clothing/pink_sweatsuit.png',
        65
    ),(
        'Purple',
        'https://uni-klaus.s3.us-west-2.amazonaws.com/clothing/purple_sweatsuit.png',
        65
    ),(
        'Red',
        'https://uni-klaus.s3.us-west-2.amazonaws.com/clothing/red_sweatsuit.png',
        65
    ),(
        'Royal Blue',
        'https://uni-klaus.s3.us-west-2.amazonaws.com/clothing/royal_blue_sweatsuit.png',
        65
    ),(
        'Yellow',
        'https://uni-klaus.s3.us-west-2.amazonaws.com/clothing/yellow_sweatsuit.png',
        65
    );
...
SELECT
    ...
    size_list,
    coalesce(
        'BONUS: ' || headband_description || ' & ' || wristband_description,
...
SELECT
    ...
    $1:trail_name::TEXT AS trail_name,
    $1:latitude::NUMBER(11, 8) AS lng,
    $1:longitude::NUMBER(10, 8) AS lat
FROM
    @MELS_SMOOTHIE_CHALLENGE_DB.TRAILS.TRAILS_PARQUET (
        file_format => MELS_SMOOTHIE_CHALLENGE_DB.TRAILS.FF_PARQUET
    );
SELECT
    TOP 100 lng || ' ' || lat coord_pair,
    'POINT(' || coord_pair || ')' AS trail_point
FROM
    MELS_SMOOTHIE_CHALLENGE_DB.TRAILS.CHERRY_CREEK_TRAIL;

CREATE
    OR REPLACE VIEW MELS_SMOOTHIE_CHALLENGE_DB.TRAILS.CHERRY_CREEK_TRAIL AS
SELECT
    $1:sequence_1 AS point_id,
    $1:trail_name::TEXT AS trail_name,
    $1:latitude::NUMBER(11, 8) AS lng,
...
SELECT
    $1:features[0]:properties:Name::TEXT AS feature_name,
    $1:features[0]:geometry:coordinates::TEXT AS feature_coordinates,
    $1:features[0]:geometry::TEXT AS geometry,
    $1:features[0]:properties::TEXT AS feature_properties,
    $1:crs:properties:name::TEXT AS specs,
    $1 AS whole_object
...
AND (name ilike '%jamba%' OR name ilike '%juice%'
...
SELECT 
    name, 
    cuisine, 
    mels_smoothie_challenge_db.locations.DISTANCE_TO_MC(coordinates) distance_from_melanies
FROM competition
ORDER BY distance_from_melanies;
